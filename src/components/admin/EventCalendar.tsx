...
import { ChevronLeft, ChevronRight, MapPin, Clock, Trash2, Calendar, Share2, Copy, Check } from "lucide-react";
...
const EventCalendar = ({ events, onEventUpdate }: EventCalendarProps) => {
  ...
  const copyToClipboard = async (url: string) => {
    try {
      ...
      toast({
        title: "Link kopiert",
        description: "Kalender-URL wurde in die Zwischenablage kopiert",
      });
              ...
              <DropdownMenuContent align="end" className="w-72">
                <DropdownMenuItem onClick={() => copyToClipboard(calendarFeedUrl)} className="cursor-pointer">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-gray-500">Für Apple Kalender, Outlook, etc.</span>
                  </div>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => window.open(`https://calendar.google.com/calendar/r?cid=${encodeURIComponent(calendarFeedUrl)}`, '_blank')} className="cursor-pointer">
                  <Calendar className="w-4 h-4 mr-2" />
                  <div className="flex flex-col">
                    <span className="font-medium">Google Kalender öffnen</span>
                    <span className="text-xs text-gray-500">Direkt in Google Calendar abonnieren</span>
                  </div>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
